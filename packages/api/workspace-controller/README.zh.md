...
对于托管部署，`requiredWorkspacePath` 会在 Host 启动期间创建并登记一个 Workspace，即使已经存在 Session 历史也是如此。该登记使首次使用初始化不再符合条件，因此 Web 客户端使用托管 Workspace，而不会在 Documents 下再创建目录。控制器在本次进程生命周期内拒绝移除该登记。`workspaceRootDirectory` 在规范化路径解析后，把后续每个 `workspace.create` 请求限制在该目录或其后代中，因此软链接无法跳出所配置的根。
