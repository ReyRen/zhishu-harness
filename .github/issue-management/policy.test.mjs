...
test('keeps trusted preflight before token minting and policy active on the official repository', () => {
  ...
  // The fork has no official Issue policy App credentials. The policy still
  // runs unconditionally in the official repository, while this guard skips
  // only the fork's copy of the workflow.
  const jobHeader = job.slice(0, job.indexOf('    steps:'))
  assert.match(jobHeader, /    if: \$\{\{ github\.repository == 'deepseek-ai\/deepseek-harness' \}\}/)
  assert.equal((jobHeader.match(/    if:/g) ?? []).length, 1)
